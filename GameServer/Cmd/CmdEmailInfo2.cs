...
namespace Pangya_GameServer.Cmd
{
    internal class CmdEmailInfo2 : Pangya_DB
    {
            ...
            if (m_ei.id <= 0)
            {
                ...
                if (is_valid_c_string(_result.data[3]))
                {
                    ...
                    catch (exception e)
                    {
                        ...
                        _smp.message_pool.push(new message("[CmdEmailInfo2::lineResult][ErrorSystem][Teste com o try para nao sair do cmd db] " + e.getFullMessageError(), type_msg.CL_FILE_LOG_AND_CONSOLE));
                    }
                }
        ...
        protected override Response prepareConsulta()
        {
            ...
            var r = procedure(
                m_szConsulta,
                Convert.ToString(m_uid) + ", " + Convert.ToString(m_email_id));

            checkResponse(r, "nao conseguiu pegar o Email[ID=" + Convert.ToString(m_email_id) + "] information do Player[UID=" + Convert.ToString(m_uid) + "]");
