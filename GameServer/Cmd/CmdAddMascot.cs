...
namespace Pangya_GameServer.Cmd
{
    public class CmdAddMascot : Pangya_DB
    {
        ...
        protected override Response prepareConsulta()
        {
            ...
            var r = procedure(
                m_szConsulta,
                Convert.ToString(m_uid) + ", " + Convert.ToString(m_mi._typeid) + ", " + Convert.ToString(m_mi.tipo) + ", " + Convert.ToString((ushort)m_mi.is_cash) + ", " + Convert.ToString(m_time) + ", " + _db.makeText(m_mi.message) + ", " + Convert.ToString(m_mi.price));
