...
// C++ TO C# CONVERTER TASK: The following line was determined to be a copy assignment (rather than a reference assignment) - this should be verified and a 'CopyFrom' method should be created:
 m_cei = _cei;
			...
			protected override Response prepareConsulta()
			{
				...
				{
					throw new exception("[CmdUpdateCardSpecialTime::prepareConsulta][Error] m_cei[index=" + Convert.ToString(m_cei.index) + ", TYPEID=" + Convert.ToString(m_cei._typeid) + "] is invalid", ExceptionError.STDA_MAKE_ERROR_TYPE(STDA_ERROR_TYPE.PANGYA_DB,
						4, 0));
				}

				var r = procedure(
					m_szConsulta,
					Convert.ToString(m_uid) + ", " + Convert.ToString(m_cei.index) + ", " + Convert.ToString(m_cei._typeid) + ", " + Convert.ToString(m_cei.efeito) + ", " + Convert.ToString(m_cei.efeito_qntd) + ", " + Convert.ToString(m_cei.tipo) + ", " + _db.makeText(UtilTime.FormatDate(m_cei.end_date.ConvertTime())));

				checkResponse(r, "nao conseguiu atualizar tempo do Card Special[index=" + Convert.ToString(m_cei.index) + ", TYPEID=" + Convert.ToString(m_cei._typeid) + ", EFEITO{TYPE: " + Convert.ToString(m_cei.efeito) + ", QNTD: " + Convert.ToString(m_cei.efeito_qntd) + "}, TIPO=" + Convert.ToString(m_cei.tipo) + ", DATE{REG_DT: " + _formatDate(m_cei.use_date.ConvertTime()) + ", END_DT: " + _formatDate(m_cei.end_date.ConvertTime()) + "}] do player[UID=" + Convert.ToString(m_uid) + "]");
