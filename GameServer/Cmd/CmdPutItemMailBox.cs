...
namespace GameServer.Cmd
{
    public class CmdPutItemMailBox : Pangya_DB
    {
            ...
            EmailInfo.item _item)
        {
            ...
            m_item.id = (int)_item.id;
            m_item._typeid = _item._typeid;
            m_item.flag_time = _item.flag_time;
            m_item.c[0] = (ushort)((ushort)(m_item.qntd = (uint)_item.qntd));
            m_item.c[3] = (ushort)((ushort)_item.tempo_qntd);
        }
        ...
        protected override Response prepareConsulta()
        {
            ...
            var r = procedure(
                m_szConsulta,
                Convert.ToString(m_uid_from) + ", " + Convert.ToString(m_uid_to) + ", " + Convert.ToString(m_mail_id) + ", " + Convert.ToString(m_item.id) + ", " + Convert.ToString(m_item._typeid) + ", " + Convert.ToString((ushort)m_item.flag_time) + ", " + Convert.ToString((m_item.qntd > 0xFFu) ? m_item.qntd : m_item.c[0]) + ", " + Convert.ToString(m_item.c[3]));
