...
{
    public class CmdFindWarehouseItem : Pangya_DB
    {
        ...
        public CmdFindWarehouseItem(uint _uid,
            uint _typeid)
        {
            this.m_uid = _uid;
        ...
        protected override void lineResult(ctx_res _result, uint _index_result)
        {
            ...
            { // found
                ...
                for (i = 0; i < 5; i++)
                {
                    m_wi.c[i] = (ushort)IFNULL<uint>(_result.data[4 + i]); // 4 + 5
                }
                m_wi.purchase = (byte)IFNULL<uint>(_result.data[9]);
                ...
                m_wi.type = (sbyte)IFNULL<uint>(_result.data[14]);
                for (i = 0; i < 4; i++)
                {
                ...
                m_wi.clubset_workshop.rank = IFNULL<uint>(_result.data[36]);
        ...
        protected override Response prepareConsulta()
        {
            ...
            if (m_typeid == 0)
            {
                throw new exception("[CmdFindWarehouseItem::prepareConsulta][Error _typeid warehouse item is '0' invalid]", ExceptionError.STDA_MAKE_ERROR_TYPE(STDA_ERROR_TYPE.PANGYA_DB,
                    4, 0));
            }
        ...
        private const string m_szConsulta = "pangya.ProcFindWarehouseItem";
    }
}