...
namespace Pangya_GameServer.Game
{
    ...
    public abstract class Game
    {
        ...
        protected Player findSessionByOID(uint32_t _oid)
        {
            ...
            ref ShotSyncData _ssd)
        {
            ...
            try
            {
                ...
                Console.WriteLine("[Game::requestReadSyncShotDate][Log] " + _ssd.toString());
                if (_ssd.pang > 37000u)
                {
    ...
    corrupt_wings.Contains(_element)))
      ...
      hat_spooky_halloween.Contains(_element)))
                    ...
                    {
                        return sIff.getInstance().getItemSubGroupIdentify22(_el.Value._typeid) == 2 && _el.Value.efeito == 19;
                    });

                    if (it.Value != null)
                    {

                        _smp.message_pool.push(new message("[Game::initPlayersItemRainRate][Log] player[UID=" + Convert.ToString(s.m_pi.uid) + "] esta equipado com Card[TYPEID=" + Convert.ToString(it.Value._typeid) + ", EFEITO=" + Convert.ToString(it.Value.efeito) + ", EFEITO_QNTD=" + Convert.ToString(it.Value.efeito_qntd) + "] especial", type_msg.CL_FILE_LOG_AND_CONSOLE));

                        if (it.Value.efeito_qntd > 0)
                        {
                            m_rv.rain += it.Value.efeito_qntd;
                        }
                    }
     ...
     hat_spooky_halloween.Contains(_element)))
                    ...
                    var it = s.m_pi.v_cei.FirstOrDefault(_el =>
                    {
                        return sIff.getInstance().getItemSubGroupIdentify22(_el.Value._typeid) == 2 && _el.Value.efeito == 31;
                    });
        ...
        protected virtual PlayerGameInfo.eCARD_WIND_FLAG getPlayerWindFlag(Player _session)
        {
            ...
            {
                return (_session.m_pi.ei.char_info.id == _el.Value.parts_id && _session.m_pi.ei.char_info._typeid == _el.Value.parts_typeid) && sIff.getInstance().getItemSubGroupIdentify22(_el.Value._typeid) == 1 && (_el.Value.efeito == 3 || _el.Value.efeito == 17 || _el.Value.efeito == 13 || _el.Value.efeito == 12);
            });

            if (it.Value != null)//(it.Key != _session.m_pi.v_cei.end().Key)
            {

                _smp.message_pool.push(new message("[Game::getPlayerWindFlag][Log] player[UID=" + Convert.ToString(_session.m_pi.uid) + "] esta equipado com Card[TYPEID=" + Convert.ToString(it.Value._typeid) + ", EFEITO=" + Convert.ToString(it.Value.efeito) + ", EFEITO_QNTD=" + Convert.ToString(it.Value.efeito_qntd) + "] Caddie", type_msg.CL_FILE_LOG_AND_CONSOLE));


                switch (it.Value.efeito)
                {
                    case 3:
        ...
        protected virtual PlayerGameInfo.stTreasureHunterInfo getPlayerTreasureInfo(Player _session)
        {
            ...
            _session.m_pi.v_cei.ToList().ForEach(el =>
             {
                 ...
                 {
                     v_cei.Add((CardEquipInfoEx)el.Value);
                 }
             });
            ...
            var it = _session.m_pi.v_cei.FirstOrDefault(_el =>
            {
                return sIff.getInstance().getItemSubGroupIdentify22(_el.Value._typeid) == 2 && _el.Value.efeito == 18;
            });

            if (it.Value != null)//(it.Key != _session.m_pi.v_cei.end().Key)
            {

                _smp.message_pool.push(new message("[Game::getPlayerTreasureInfo][Log] player[UID=" + Convert.ToString(_session.m_pi.uid) + "] esta equipado com Card[TYPEID=" + Convert.ToString(it.Value._typeid) + ", EFEITO=" + Convert.ToString(it.Value.efeito) + ", EFEITO_QNTD=" + Convert.ToString(it.Value.efeito_qntd) + "] especial", type_msg.CL_FILE_LOG_AND_CONSOLE));


                pti.all_score += (byte)it.Value.efeito_qntd;
            }
        ...
        public virtual void requestCalculePang(Player _session)
        {
            ...
            pang_rate = TRANSF_SERVER_RATE_VALUE((int)pgi.used_item.rate.pang) * TRANSF_SERVER_RATE_VALUE((int)(m_rv.pang + (m_ri.modo == (byte)RoomInfo.MODO.M_SHUFFLE ? 10 : 0))) * course_rate;
...
{
    if (_el.Value.parts_id == _session.m_pi.ei.char_info.id
        && _el.Value.parts_typeid == _session.m_pi.ei.char_info._typeid
        && sIff.getInstance().getItemSubGroupIdentify22(_el.Value._typeid) == 5)
    {
        if (_el.Value.efeito == 2)
        {
            ui.rate.exp += _el.Value.efeito_qntd;
        }
        else if (_el.Value.efeito == 1)
        {
            ui.rate.pang += _el.Value.efeito_qntd;
        }
    }
    else if (_el.Value.parts_id == 0 && _el.Value.parts_typeid == 0 && sIff.getInstance().getItemSubGroupIdentify22(_el.Value._typeid) == 2)
    {
        if (_el.Value.efeito == 3)
        {
            ui.rate.exp += _el.Value.efeito_qntd;
        }
        else if (_el.Value.efeito == 2)
        {
            ui.rate.pang += _el.Value.efeito_qntd;
        }
        else if (_el.Value.efeito == 34)
        {
            ui.rate.club += _el.Value.efeito_qntd;
        }
    }
});
    ...
    motion_item.Contains(_element)))
        ...
        public virtual void update_sync_shot_achievement(Player _session, Location _last_location)
        {
            ...
            try
            {
                ...
                if (pgi.shot_sync.state_shot.display.acerto_hole == 1)
                {
                    ...
                    // Long-putt
                    if (pgi.shot_sync.state_shot.display.long_putt == 1 && pgi.shot_sync.state_shot.shot.club_putt == 1)
                    {
        ...
        public PlayerGameInfo makePlayerInfoObject(Player _session)
        {
