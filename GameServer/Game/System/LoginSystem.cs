    ...
    public class LoginSystem
    {
        ...
        public void requestLogin(Player _session, Packet _Packet)
        {
            ...
            try
            {
                ...
                if (_pi.block_flag.m_id_state.id_state.ull_IDState != 0)
                {

                    if (_pi.block_flag.m_id_state.id_state.st_IDState.L_BLOCK_TEMPORARY && (_pi.block_flag.m_id_state.block_time == -1 || _pi.block_flag.m_id_state.block_time > 0))
                    {
                    ...
                    else if (_pi.block_flag.m_id_state.id_state.st_IDState.L_BLOCK_FOREVER)
                    {

                        throw new exception("[LoginSystem::requestLogin][Log] Bloqueado permanente. player [UID=" + (_pi.uid)
                                + ", ID=" + (_pi.id) + "]");
                    }
                    else if (_pi.block_flag.m_id_state.id_state.st_IDState.L_BLOCK_ALL_IP)
                    {
                    ...
                    else if (_pi.block_flag.m_id_state.id_state.st_IDState.L_BLOCK_MAC_ADDRESS)
                    {
                ...
                //		throw exception("Player[UID=" + (_pi.uid) + "].\tAuthKey ja foi utilizada antes.", STDA_MAKE_ERROR(STDA_ERROR_TYPE::GAME_SERVER, 1056, 0));

                //	// ### Isso aqui é uma falha de segurança faltal, muito grande nunca posso deixar isso ligado depois que colocar ele online
                //	if (!m_login_manager.canSameIDLogin() &&
                //string.Compare(kol.keys[0], cmd_akli.getInfo().key) != 0
                //	)
                //		throw new exception("Player[UID=" + (_pi.uid) + "].\tAuthKey no bate(no match).", STDA_MAKE_ERROR(STDA_ERROR_TYPE::GAME_SERVER, 1057, 0));
                ...
                //		{
                //			_smp.message_pool.push(("[LoginSystem::requestLogin][WARNING] Player[UID=" + (_pi.uid) + "].\tClient Version not match. Server: "
                ...
                _pi.mi.state_flag.stFlagBit.visible = 1;
                ...
                if (_pi.m_cap.stBit.game_master.IsTrue())
                {
                    ...
                    _pi.mi.state_flag.stFlagBit.visible = _session.m_gi.visible;
                    _pi.mi.state_flag.stFlagBit.whisper = _session.m_gi.whisper;
                    _pi.mi.state_flag.stFlagBit.channel = _session.m_gi.channel;
                }
                ...
                // Entra com sucesso
                _session.Send(packet_func.pacote044(Program.gs.m_si, 0xD3));
        ...
        void SQLDBResponse(int _msg_id, Pangya_DB _pangya_db, object _arg)
        {
            ...
            try
            {
                ...
                switch (_msg_id)
                {
                    ...
                    case 2: // Member Info - User Equip
                        {
                            ...
                            _session.Send(packet_func.pacote1A9(10000/*milliseconds*/));                                   // Tempo para enviar um pacote, ant Bot
                    ...
                    case 5: // Tutorial Info
                        {
                            ...
                            _session.m_pi.TutoInfo = ((CmdTutorialInfo)(_pangya_db)).getInfo();  // cmd_tti.getInfo();
                                                                                                 //_pi.TutoInfo = pangya_db.getTutorialInfo(_pi.uid);
                    ...
                    case 6: // Coupon Gacha
                        {
                            _session.m_pi.cg = ((CmdCouponGacha)(_pangya_db)).getCouponGacha();  // cmd_cg.getCouponGacha();
                                                                                                 //_pi.cg = pangya_db.getCouponGacha(_pi.uid);
                    ...
                    case 10:    // Cookie
                        {
                            ...
                            NormalManagerDB.add(33, new CmdCaddieInfo(_session.m_pi.uid, CmdCaddieInfo.TYPE.ONE), SQLDBResponse, _session);
                    ...
                    case 12:    // Character Info
                        {
                            ...
                            if (pi.ei.char_info != null && pi.ui.getQuitRate() < GOOD_PLAYER_ICON)
                                pi.mi.state_flag.stFlagBit.icon_angel = 0; // _pi.ei.char_info.AngelEquiped();
                            else
                    ...
                    case 15:    // Warehouse Item
                        {
                            ...
                            //while ((ui_ticket_report_scroll = _pi.findUpdateItemByTypeidAndType(TICKET_REPORT_SCROLL_TYPEID, UpdateItem.UI_TYPE.WAREHOUSE)) != _pi.mp_ui.end())
                            //{

                            //    try
                            //    {

                            //        var pWi = _pi.findWarehouseItemById(ui_ticket_report_scroll.Values.id);
                            ...
                            { // ClubSet Info
                                ...
                                //    for (var i = 0u; i < (Marshal.SizeOf(_pi.ei.csi.enchant_c) / Marshal.SizeOf(_pi.ei.csi.enchant_c[0])); ++i)
                                //        _pi.ei.csi.enchant_c[i] = cs.slot[i] + it.second.clubset_workshop.c[i];

                                //}
                                //else
                                //    _smp.message_pool.push("[SQLDBResponse][Erro] player[UID=" + (_pi.uid) + "] tentou inicializar ClubSet[TYPEID="
                            ...
                            else
                            {
                                ...
                                {   // Não tem add o ClubSet padrão para ele(CV1)
                                    ...
                                    //_smp.message_pool.push("[SQLDBResponse][WARNING] Player[UID=" + (_pi.uid)
                                    ...
                                    //if (item._typeid != 0 && (item.id = item_manager.addItem(item, _session, 2/*Padrão item*/, 0)) != item_manager.RetAddItem.T_ERROR
                                    //    && (it = _pi.findWarehouseItemItById(item.id)) != _pi.mp_wi.end())
                                    //{

                                    //    _pi.ue.clubset_id = it.second.id;
                                    //    _pi.ei.clubset = &it.second;

                                    //    // Esse C do WarehouseItem, que pega do DB, não é o ja updado inicial da taqueira é o que fica tabela enchant, 
                                    //    // que no original fica no warehouse msm, eu só confundi quando fiz
                                    //    // [AJEITEI JA] (tem que ajeitar na hora que coloca no DB e no DB isso)
                                    //    _pi.ei.csi = { it.second.id, it.second._typeid, it.second.c };
                    ...
                    case 18:    // Check if have Achievement
                        {
                            ...
                            //    _pi.mgr_achievement.initAchievement(_pi.uid, true/*Create sem verifica se o player tem achievement, por que aqui ele já verificou);
                    ...
                    case 19:    // Achievement Info
                        {
                            ...
                            //_pi.mgr_achievement.initAchievement(_pi.uid, cmd_ai.getInfo());
        ...
        void sendCompleteData(Player _session)
        {
            ...
            try
            {
               ...
               // _session.Send(packet_func.pacote135()); 
                ...
                //_session.Send(packet_func.pacote169(_pi.ti_current_season, 5/*season atual*/);
                //_session.Send(p);

                //_session.Send(packet_func.pacote169(_pi.ti_rest_season);
                //_session.Send(p);

                //_session.Send(packet_func.pacote0B4(_pi.v_tsi_current_season, 5/*season atual*/);
                //_session.Send(p);

                //_session.Send(packet_func.pacote0B4(_pi.v_tsi_rest_season);
                //_session.Send(p);

                //_session.Send(packet_func.pacote158(_pi.uid, _pi.ui, 0);
                //_session.Send(p);    // Total de season, 5 atual season

                //_session.Send(packet_func.pacote25D(_pi.v_tgp_current_season, 5/*season atual*/);
                //_session.Send(p);

                //_session.Send(packet_func.pacote25D(_pi.v_tgp_rest_season, 0);
                //_session.Send(p);
