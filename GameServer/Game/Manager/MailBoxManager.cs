    ...
    public class MailBoxManager
    {
            ...
            ref string _msg)
        {
            ...
            if (index != -1)
            {
                ...
                message_pool.push(new message("[MailBoxManager::_sendMessage][Log] replace string para[str=" + _msg + "] por que tinha valores que o MSSQL nao aceita", type_msg.CL_FILE_LOG_AND_CONSOLE));
            }
            ...
            Cmd.CmdAddMsgMail cmd_amm = new CmdAddMsgMail(_from_uid, _to_uid, _msg);

            NormalManagerDB.add(0, cmd_amm, null, null);
...
#if DEBUG
            message_pool.push(new message("[MailBoxManager::putItemInMail][Log] player[UID=" + Convert.ToString(_from_uid) + "] colocou item[TYPEID=" + Convert.ToString(_item._typeid) + ", ID=" + Convert.ToString(_item.id) + ", QNTD=" + Convert.ToString(_item.qntd > 0xFFu ? _item.qntd : _item.c[0]) + "] no mail[ID=" + Convert.ToString(_mail_id) + "] do player[UID=" + Convert.ToString(_to_uid) + "]", type_msg.CL_FILE_LOG_AND_CONSOLE));
#else
				message_pool.push(new message("[MailBoxManager::putItemInMail][Log] player[UID=" + Convert.ToString(_from_uid) + "] colocou item[TYPEID=" + Convert.ToString(_item._typeid) + ", ID=" + Convert.ToString(_item.id) + ", QNTD=" + Convert.ToString(_item.qntd > 0xFFu ? _item.qntd : _item.c[0]) + "] no mail[ID=" + Convert.ToString(_mail_id) + "] do player[UID=" + Convert.ToString(_to_uid) + "]", type_msg.CL_ONLY_FILE_LOG));
#endif
        }
            ...
            uint _count)
        {
            ...
            if (_pItem == null)
            {
                throw new exception("[MailBoxManager::putItemInMail][Error] _pItem is nullptr", ExceptionError.STDA_MAKE_ERROR_TYPE(STDA_ERROR_TYPE.MAIL_BOX_MANAGER,
                    7, 0));
            }
            ...
            EmailInfo.item _item)
        {
            ...
            // Cmd add item
            Cmd.CmdPutItemMailBox cmd_pimb = new CmdPutItemMailBox(_from_uid, // Waiter
                _to_uid, _mail_id, _item);
                ...
                object _arg)
        {
...
#if DEBUG
                // Static class
                message_pool.push(new message("[MailBoxManager::SQLDBResponse][WARNING] _arg is nullptr na msg_id = " + Convert.ToString(_msg_id), type_msg.CL_FILE_LOG_AND_CONSOLE));
#endif // _DEBUG
                return;
            }
            ...
            {
                message_pool.push(new message("[MailBoxManager::SQLDBResponse][Error] " + _pangya_db.getException().getFullMessageError(), type_msg.CL_FILE_LOG_AND_CONSOLE));
                return;
            }
                ...
                case 1: // Insert Command
                    {
                        ...
                        message_pool.push(new message("[MailBoxManager::SQLDBResponse][Log] Adicionou Command[" + cmd_ic.getInfo().ToString() + "] com sucesso!", type_msg.CL_FILE_LOG_AND_CONSOLE));
